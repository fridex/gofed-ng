...
class AppSingleton(object):
    ...
    class Configuration(object):
        def __init__(self, db_host, db_port, db_name, db_collection_name_project, db_collection_name_package):
            ...
            self._deps_project = self._db[db_collection_name_project]
            self._deps_package = self._db[db_collection_name_package]

        def get_deps_collection_project(self):
            return self._deps_project

        def get_deps_collection_package(self):
            return self._deps_package
...
@app.route('/deps/v1/project/deps/<project>/<commit>', methods=['GET'])
def deps_project_deps(project, commit):
    ret = []
    filtering = {'_id': 0}

    cursor = conn.get_deps_collection_project().find({'project': project, 'commit': commit}, filtering)
    for item in cursor:
        ret.append({'commit': item['commit'], 'commit-date': item['commit-date'],
                    'meta': item['meta'], 'deps': item['deps']})

    if len(ret) == 0:
        abort(404)

    return jsonify({'project': project, 'commit': commit, 'deps': ret})


@app.route('/deps/v1/project/log/<project>', methods=['GET'])
def deps_project_log(project):
    ret = []
    filtering = {'_id': 0, 'deps': 0}

    cursor = conn.get_deps_collection_project().find({'project': project}, filtering)
    for item in cursor:
    ...
    return jsonify({'project': project, 'log': ret})


@app.route('/deps/v1/project/listing', defaults={'page': 1}, methods=['GET'])
@app.route('/deps/v1/project/listing/<int:page>', methods=['GET'])
def deps_project_listing(page):
    ret = []
    filtering = {'commit': 0, '_id': 0, 'deps': 0, 'meta': 0, 'commit-date': 0}

    cursor = conn.get_deps_collection_project().find({}, filtering)
    for item in cursor:
...
@app.route('/deps/v1/package/deps/<package>/<version>/<distro>', methods=['GET'])
def deps_package_deps(package, version, distro):
    ret = []
    filtering = {'_id': 0, 'version': 0, 'distro': distro, 'package': 0}

    cursor = conn.get_deps_collection_package().find({'package': package, 'version': version, 'distro': distro},
                                                     filtering)
    for item in cursor:
        ret.append({'meta': item['meta'], 'deps': item['deps']})

    if len(ret) == 0:
        abort(404)

    return jsonify({'package': package, 'version': version, 'distro': distro, 'deps': ret})


@app.route('/deps/v1/package/log/<package>', defaults={'page': 1}, methods=['GET'])
@app.route('/deps/v1/package/log/<package>/<int:page>', methods=['GET'])
def deps_package_log(package, page):
    ret = []
    filtering = {'_id': 0, 'deps': 0}

    cursor = conn.get_deps_collection_package().find({'package': package}, filtering)
    for item in cursor:
...
@app.route('/deps/v1/project/listing', defaults={'page': 1}, methods=['GET'])
@app.route('/deps/v1/project/listing/<int:page>', methods=['GET'])
def deps_package_listing(page):
    ret = []
    filtering = {'_id': 0, 'deps': 0, 'meta': 0, 'version': 0, 'distro': 0}
...
def print_help(progname):
    ...
    print >> sys.stderr, "Dependencies REST service"
